...
def main():
    ...
    parser.add_argument('image_path2', type=str, help='Input Image file.')
    parser.add_argument(
        '--output-file',
        type=str,
    ...
    pytorch2onnx(
        model,
        model_type=args.model_type,
        output_file='tmp2.onnx',
        img_path=args.image_path2,
        opset_version=args.opset_version,
        verify=args.verify,
        verbose=args.verbose,
        show=args.show,
...
if __name__ == '__main__':
    ...
    import onnx
    onnx_model1 = onnx.load('tmp1.onnx')
    onnx_model2 = onnx.load('tmp2.onnx')
    for i, (node1, node2) in enumerate(
            zip(onnx_model1.graph.node, onnx_model2.graph.node)):
        assert node1 == node2
    onnx_model1 == onnx_model2
